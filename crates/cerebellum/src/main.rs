//! Cerebellum Service - Error Correction and Skill Refinement
//!
//! Responsibilities:
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("cerebellum=info".parse().unwrap()),
        )
        .init();

    let mut config = RegionConfig::default();
    config.base.service_name = "cerebellum".to_string();

    info!(
        service = %config.base.service_name,
        version = %config.base.version,
        "Starting Cerebellum service"
    );
...
async fn store(
    ...
    info!(experience_id = %experience_id, "Storing error delta");

    Ok(Json(StoreResponse {
        experience_id,
        stored: true,
        region: state.config.base.service_name.clone(),
        message: Some("Error delta stored".to_string()),
    }))
}
...
async fn query(
    ...
    info!(limit = request.limit, "Querying micro-policies");
